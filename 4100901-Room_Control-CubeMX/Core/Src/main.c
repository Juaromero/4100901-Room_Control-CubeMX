...
#include <stdio.h>
#include <string.h>  // lo uso para strncmp y memset
...
// Yo defino la clave. Son 4 teclas. Si quiero otra, la cambio aquí.
static const char ACCESS_CODE[5] = "1*3A";  
// Aquí guardo lo que el usuario va tecleando. Pongo espacio para 4 + '\0'.
static char entered[5] = {0};
// Este índice me dice cuántas teclas ya tengo (0 a 4).
static uint8_t idx = 0;

// Estas funciones me simplifican encender y apagar el LED_EXT.
// Prefiero tenerlas así para no repetir HAL_GPIO_WritePin en todo lado.
static inline void led_ext_on(void)  { HAL_GPIO_WritePin(LED_EXT_GPIO_Port, LED_EXT_Pin, GPIO_PIN_SET); }
static inline void led_ext_off(void) { HAL_GPIO_WritePin(LED_EXT_GPIO_Port, LED_EXT_Pin, GPIO_PIN_RESET); }

// Esta función me parpadea el LED varias veces. La uso cuando la clave es incorrecta.
static void led_ext_blink(uint8_t times, uint16_t period_ms) {
  for (uint8_t i = 0; i < times; i++) {
    led_ext_on();  HAL_Delay(period_ms);
    led_ext_off(); HAL_Delay(period_ms);
  }
}
/* USER CODE END PV */
...
int main(void)
{
  ...
  keypad_init(&keypad);
  printf("Sistema listo. Ingresa la clave de acceso...\r\n");
  ...
  if (ring_buffer_read(&keypad_rb, &key_from_buffer)) {
    // Aquí saco una tecla del buffer (la ISR ya la metió antes).
    char k = (char)key_from_buffer;

    // Imprimo lo que se presiona solo para ver que llega algo. No es obligatorio.
    printf("Tecla: %c\r\n", k);

    // Voy guardando las teclas hasta tener 4. Uso idx para saber cuántas llevo.
    if (idx < 4) {
      entered[idx++] = k;
    }

    // Cuando ya tengo 4, hago la verificación.
    if (idx == 4) {
      // Cierro el string por si quiero imprimirlo. No es necesario para strncmp, pero me gusta dejarlo bien.
      entered[4] = '\0';

      // Comparo lo que se tecleó contra mi clave. Si coincide, es acceso permitido.
      if (strncmp(entered, ACCESS_CODE, 4) == 0) {
        // Enciendo el LED fijo un momento para indicar que está bien.
        printf("ACCESO PERMITIDO\r\n");
        led_ext_on();
        HAL_Delay(3000);
        led_ext_off();
      } else {
        // Si no coincide, hago parpadear el LED para avisar que está mal.
        printf("ACCESO DENEGADO\r\n");
        led_ext_blink(3, 150);
      }

      // Limpio el buffer y reinicio el índice para esperar la próxima clave.
